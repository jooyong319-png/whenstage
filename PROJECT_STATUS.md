...
- 2026-09-21: 외부 SEO 점검 스펙 처리(robots `/_next/` 차단 제거 · EN/JA 한국어 날짜 버그 · `location.address` PostalAddress · sitemap `updated_at` 신설 · 루트 리다이렉트 `Vary` · 언론사 사진 21건 제거 · 위키미디어 CC 출처 표기 · `game-detail`→`concert-detail`) + **EN 리서처를 해외 팬 대상 한국 공연으로 전환**(검색어 실측: `seoul concerts october 2026` 류가 전부 0클릭이었고 en에 한국 공연이 1건뿐이었다) + Vercel 한도 초과 대응(엣지 캐시 `s-maxage` · `should-build.mjs`의 조용한 배포 스킵 수정 · 배포 20→5개 · RSC 페이로드 79% 감소). 경위는 `wiki/decisions.md` 2026-09-21.
- 2026-07-25: [개발] `20260725-04` 완료 — 홈·인덱스·이미지없는 상세 페이지 `og:image` 누락 수정(커밋 d07dbc5). Next.js App Router가 자식 `generateMetadata`의 `openGraph`를 부모와 얕은 병합(통째 대체)해 레이아웃의 `images:['/og-image.png']`가 사라지던 문제(라이브 ko/en/ja 홈에 og:image 부재·twitter:image만 정상)를, `openGraph`를 재정의하는 로케일 페이지 10곳에서 고유 이미지 없을 때 기본 OG 이미지로 폴백하도록 수정. `lib/i18nLabels.ts`에 `DEFAULT_OG_IMAGE` 상수 신설(리스트/홈 6곳 `images:[DEFAULT_OG_IMAGE]` 명시, 상세 4곳 `undefined`→기본이미지 폴백). `concert/[id]`는 기존 폴백 보유라 불변, `twitter`는 상속 유지라 무변경. 저위험 §5-A(메타데이터 리터럴)라 타입체크 ✅(`/tmp` 디스크풀로 `/sessions`에 클린설치해 `tsc --noEmit` 통과) + 코드 리뷰(undefined 잔존 0·DEFAULT_OG_IMAGE 10곳·concert diff 0), 전체 빌드는 Vercel 프리뷰 위임(재배포 후 `curl|grep og:image` 육안 확인 권장).
