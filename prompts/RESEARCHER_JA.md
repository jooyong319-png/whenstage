...
**2026-07-30 실측(ja)**: 공연장 **50개 중 39개가 1건짜리**, 아티스트 **36명 중 35명이 1건짜리**.
색인되는 모아보기 페이지는 12개(3개 로케일 중 가장 많지만 여전히 적다). 공연장이 50개로 가장 많이
흩어져 있는데, **全国ツアー가 도시마다 다른 会場을 쓰기 때문**이다.

**⚠️ 함정 — "全国ツアー의 다른 회차"만 채우면 공연장 페이지는 안 늘어난다**
백필 1건은 아티스트 페이지와 공연장 페이지를 **동시에** 건드린다. 같은 투어의 다른 도시 회차를 채우면
아티스트 페이지는 두꺼워지지만 **공연장은 매번 새 1건짜리가 생겨** noindex 페이지가 오히려 늘어난다.
일본은 전국 투어가 많아 이 함정에 빠지기 쉽다. 두 목표는 서로 다른 전략을 요구한다:
- **아티스트 페이지 전환** → 같은 **아티스트**의 다른 공연 (공연장은 아무거나)
- **공연장 페이지 전환** → 같은 **공연장**의 다른 공연 (아티스트는 아무거나)

**⚠️ 먼저 할 일 — "1건짜리 목록"을 뽑아라**
백필 후보를 검색하기 전에 `concerts.ja.json`을 훑어서 **현재 1건뿐인 공연장 목록**을 만들어라.
이 목록 없이 아무 과거 공연이나 채우면 효과가 거의 없다.

**대상 선정 (우선순위대로, 과거 2년 이내)**
1. **이미 등록된 대형 회장에서 열린 "다른 아티스트"의 과거 공연** ← ja에서 가장 중요하다.
...
2. 대형 페스티벌의 지난 회차(フジロック·サマーソニック·ROCK IN JAPAN·COUNTDOWN JAPAN 등) — 매년 같은
   장소에서 열려서 **공연장·아티스트를 동시에** 두껍게 한다
3. **이미 등록된 아티스트**(`developer`)의 과거 일본 공연 — 단 이 경우 **가능하면 이미 등록된 会場에서
...
24. **과거 백필(3-B단계)은 예정 공연 리서치를 끝낸 뒤에만**, 사이클당 최대 8건. 기존 아티스트·공연장을 두껍게 하는 방향으로 고를 것(무작위 과거 공연 금지). 일본어 위키피디아 `ライブ・ツアー` 섹션과 공식 사이트 LIVE 아카이브가 가장 효율적인 소스다. 백필 항목은 `release_date_approx:false` + 티켓팅 필드 비움 + 과거 시제 description + **등록 직전 `release_date`가 오늘보다 과거인지 확인**(AGENTS.md §7)
