...
4. 🔴 **언론사 보도사진 금지.** 아래 도메인은 **절대 `image_url`에 넣지 않는다**:
   `khan.co.kr` · `tenasia.co.kr` · `heraldcorp.com` · `starnewskorea.com` · `etoday.co.kr` ·
   `mdtoday.co.kr` · `emetro.co.kr` · `ibabynews.com` — 그 외 신문·방송사 도메인 전부 포함.

   2026-09-21에 이런 사진 **21건이 실제로 들어와 있어 전부 뺐다**(티파니 영·라이즈·레드벨벳·
   에스파·NCT 127·베이비몬스터 등). 한국 언론사는 사진 저작권을 적극적으로 주장하고,
   핫링크는 저작권 문제와 별개로 상대가 막으면 한꺼번에 깨진다.

   **쓸 수 있는 것**: 위키미디어 커먼즈(`upload.wikimedia.org`) · 예매처 공식 포스터
   (인터파크·예스24·멜론티켓) · 소속사 공식 배포 이미지.
   확신이 안 서면 **이미지를 넣지 않는다** — 빈 값이면 화면이 폴백으로 잘 처리된다.
