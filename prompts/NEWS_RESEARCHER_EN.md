...
- **매주 수요일 14:00** 깨어남 (2026-09-12 전환 전엔 하루 2회였다)
  - 리서처가 09시대에 먼저 돌고 나서야 이 시각이다 — 그날 아침 추가된 공연까지 반영된다.
    **이 순서는 지켜야 한다**(브리핑은 `concerts.en.json`만 읽으므로).
  - KO는 월요일, JA는 금요일 14:00 — 요일을 흩은 건 `CHAT.md` append 충돌을 피하기 위해서다
  - 스케줄이 더 자주 잡혀 있어도 2단계의 6일 가드가 막으니 그냥 종료하면 된다
...
CHAT.md는 append 위치가 자주 충돌한다 — 브리핑(`content/news/*.en.md`)을 CHAT.md와
...
git commit -m "[EN 브리핑] $(date '+%Y-%m-%d') 주간 브리핑 — <각도>"
git fetch origin
...
```
## [YYYY-MM-DD HH:MM] [EN 브리핑]
주간 브리핑 작성 완료 (영어권/글로벌)
- 각도: <This week's on-sales / This week's shows / Next month preview / Just went on sale>
- 제목: <제목>
- 다룬 공연 N건 (전부 /en/concert/<id>로 링크)
