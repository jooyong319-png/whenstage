...
**2026-07-30 실측(ko)**: 공연장 16개 중 **10개가 1건짜리**, 아티스트 28명 중 **26명이 1건짜리**.
즉 색인되는 모아보기 페이지가 8개뿐이다. **1건짜리에 1건만 더 붙이면 그 페이지가 색인 대상으로 바뀐다.**

**⚠️ 먼저 할 일 — "1건짜리 목록"을 뽑아라**
백필 후보를 검색하기 전에 `concerts.ko.json`을 훑어서 **현재 1건뿐인 공연장·아티스트 목록**을 만들어라.
이 목록 없이 아무 과거 공연이나 채우면 효과가 거의 없다.

**⚠️ 함정 — "같은 투어의 다른 날짜"만 채우면 공연장 페이지는 안 늘어난다**
백필 1건은 아티스트 페이지와 공연장 페이지를 **동시에** 건드린다. 같은 아티스트의 다른 도시 공연을 채우면
아티스트 페이지는 두꺼워지지만 **공연장은 매번 새 1건짜리가 생겨** noindex 페이지가 오히려 늘어난다.
두 목표는 서로 다른 전략을 요구한다:
- **아티스트 페이지 전환** → 같은 **아티스트**의 다른 공연 (공연장은 아무거나)
- **공연장 페이지 전환** → 같은 **공연장**의 다른 공연 (아티스트는 아무거나)

**대상 선정 (우선순위대로, 과거 2년 이내)**
1. **대형 공연장에 과거 공연을 몰아 넣기** — KSPO DOME·고척스카이돔·잠실종합운동장·올림픽공원·
   인스파이어 아레나 등. 한국은 서울 몇 개 공연장에 공연이 집중되니 후보가 풍부하고, 한 사이클로
   그 공연장 페이지를 확실히 2건 이상으로 만들 수 있다. **효율이 가장 높다.**
2. **이미 등록된 아티스트**(`developer`)의 과거 한국 공연 — 1건짜리 아티스트를 2건으로 만든다
3. 정기 페스티벌의 지난 회차(펜타포트·서울재즈페스티벌 등) — 회차마다 같은 공연장을 쓰는 경우가
   많아 공연장·아티스트를 동시에 두껍게 한다
...
23. **과거 백필(3-B단계)은 예정 공연 리서치를 끝낸 뒤에만**, 사이클당 최대 8건. 기존 아티스트·공연장을 두껍게 하는 방향으로 고를 것(무작위 과거 공연 금지). 백필 항목은 `release_date_approx:false` + 티켓팅 필드 비움 + 과거 시제 description + **등록 직전 `release_date`가 오늘보다 과거인지 확인**(AGENTS.md §7)
