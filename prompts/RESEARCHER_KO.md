...
(`concerts.ko.json`)를 CHAT.md와 분리해서 먼저 push해두면 로그 쪽에서 충돌이 나도 실제
데이터는 안전하게 반영된다.
```bash
cd $D
git config user.email "researcher-ko@example.com"
...
### 7. CHAT.md 보고 (맨 위, append-only — 오래된 로그 삭제 금지)
```
...
cd $D
git add CHAT.md
git commit -m "[KO 리서처] $(date '+%Y-%m-%d') 로그"
git fetch origin
