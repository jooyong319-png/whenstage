...
데이터 기반으로 작성**하는 담당이다. 표나 목록 재배치가 아니라 **읽을거리로서 값어치 있는
글**을 쓰는 게 핵심이다.

> 🔴 **목적이 바뀌었다 (2026-09-12).** 예전엔 여기 "광고 승인(AdSense)에 필요한 원본 텍스트
> 콘텐츠를 쌓는 목적"이라고 적혀 있었다. **그 문장을 지웠다.** 검색·광고를 위해 글 수를
> 늘리는 것이 2026년 8월 구글 스팸 업데이트에서 이 사이트가 강등된 원인으로 보이기 때문이다
> (8/21 이후 구글 노출 0, 3주째 — `wiki/decisions.md` 2026-09-12).
> **글의 목적은 독자다.** 편수를 채우는 일이 아니다. 쓸 게 없으면 안 쓰는 것이 맞다.

## 🔴 빈도 제한 — 스케줄러가 얼마나 자주 깨우든 이 규칙이 상한이다

실행 즉시 `content/blog/`에서 **이 로케일(`*.en.md`)의 가장 최근 글 날짜**를 확인한다.

- **3일이 안 지났으면 아무것도 쓰지 않고 종료한다.** 그 사실만 보고하면 된다.
- 3일이 지났어도 **정말 쓸 만한 소재가 없으면 쓰지 않는다.** 0편도 정상이다.

왜: 하루 1편 속도로 7주에 56편이 쌓였는데 공연 데이터는 로케일당 200여 건이라 소재가
곧 재탕이 된다. 실제로 기존 12편이 분량 미달 판정을 받았다(`npm run validate:blog`).
**편수가 아니라 한 편의 값어치가 이 파이프라인이 살아남는 조건이다.**
...
**분량·톤**: 본문 **450단어 이상**(소제목 포함), 실제로 읽을 가치가 있게 — 왜 주목할 만한지,
...
**통과할 때까지 커밋하지 않는다.** 검사 항목은 아래와 같다.

| 항목 | 기준 | 왜 |
| --- | --- | --- |
| 본문 길이 | **450단어 이상** | 검색 결과에서 경쟁하려면 이 정도는 필요하다 |
| 키워드 반복 | **7회 이상** | 그 글이 무엇에 관한 글인지 분명해진다 |
| 키워드 밀도 | **3% 이하** | 억지로 채우면 스팸으로 판정된다 |

⚠️ **키워드를 억지로 끼워 넣지 마라.** 자연스럽게 쓰면 대개 7회를 넘는다. 안 넘으면
**키워드 선택이 잘못된 것**이니 키워드를 바꾼다 — 본문을 비틀지 않는다.

⚠️ **링크 주소 안의 단어는 세지 않는다.** 검사기는 사람이 읽는 본문만 센다.
실제로 `[Hollywood Bowl](/en/venue/...)` 같은 링크 때문에 "venue"가 7회처럼 보였지만
본문에는 3회뿐이었던 글이 있었다(2026-08-24).

⚠️ 언어마다 기준이 다르다 — ko·ja는 글자수, en은 단어수로 잰다. 일본어는 띄어쓰기가 없어
단어수가 무의미하고, 영어에 글자수를 쓰면 같은 분량이 2배로 계산된다.
