...
- `data/artist-bios.json`의 bio 보유 아티스트는 3명뿐 → **사실상 둘 다 "2건 이상" 기준**이다.
- 공연장은 `concert_tour`/`festival`/`fanmeeting`에서만 뽑힌다 — `music_release`의 `platforms`는 공연장이 아니다.

**⚠️ 2026-07-30 실측(en) — 3개 로케일 중 가장 나쁘다**: 공연장 **32개 중 31개가 1건짜리**,
아티스트 **36명 중 35명이 1건짜리**. 색인되는 모아보기 페이지가 **단 2개**다.
...
백필 1건은 아티스트 페이지와 공연장 페이지를 **동시에** 건드린다. 같은 투어의 다른 도시 스탑을 채우면
아티스트 페이지는 두꺼워지지만 **공연장은 매번 새 1건짜리가 생겨** noindex 페이지가 오히려 늘어난다.
북미 투어 특성상 EN에서 이 함정이 가장 크다. 두 목표는 서로 다른 전략을 요구한다:
- **아티스트 페이지 전환** → 같은 **아티스트**의 다른 공연 (공연장은 아무거나)
- **공연장 페이지 전환** → 같은 **공연장**의 다른 공연 (아티스트는 아무거나)

**⚠️ 먼저 할 일 — "1건짜리 목록"을 뽑아라**
백필 후보를 검색하기 전에 `concerts.en.json`을 훑어서 **현재 1건뿐인 공연장 목록**을 만들어라.
이 목록 없이 아무 과거 공연이나 채우면 효과가 거의 없다.

**대상 선정 (우선순위대로, 과거 2년 이내)**
1. **이미 등록된 공연장에서 열린 "다른 아티스트"의 과거 공연** ← EN에서 가장 중요하다.
   특히 상시 공연이 많은 대형 회장을 노려라: Madison Square Garden, Hollywood Bowl, Red Rocks,
   The Forum, Barclays Center, O2 Arena 등. 이런 곳은 과거 공연 후보가 무한하니 한 사이클로
   그 공연장 페이지를 2건 이상으로 확실히 만들 수 있다
2. 대형 페스티벌의 지난 회차(Coachella·Lollapalooza·Glastonbury·Bonnaroo 등) — 매년 같은 장소에서
   열려서 **공연장·아티스트를 동시에** 두껍게 한다. EN에서 특히 효율이 좋다
3. **이미 등록된 아티스트**(`developer`)의 과거 공연 — 단 이 경우 **가능하면 이미 등록된 공연장에서
   열린 것**을 고를 것. 그래야 새 1건짜리 공연장을 만들지 않는다
- ❌ 위 1~3에 없는 무작위 과거 공연은 건너뛴다. **1건짜리 공연장을 새로 만드는 백필은 순 손실에 가깝다** —
  상세 페이지 1개가 늘지만 noindex 페이지도 1개 늘어난다.
...
- 과거 백필 B개 (아티스트 강화 a / 공연장 강화 b / 대형·페스티벌 c) — 건너뛰었으면 "0개(사유)"
...
23. **과거 백필(3-B단계)은 예정 공연 리서치를 끝낸 뒤에만**, 사이클당 최대 8건. 기존 아티스트·공연장을 두껍게 하는 방향으로 고를 것(무작위 과거 공연 금지). Wikipedia 투어 문서의 `Tour dates` 표가 가장 효율적인 소스다. 백필 항목은 `release_date_approx:false` + 티켓팅 필드 비움 + 과거 시제 description + **등록 직전 `release_date`가 오늘보다 과거인지 확인**(AGENTS.md §7)
