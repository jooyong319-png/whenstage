...
## 이후 추가된 것 (2026-09-08 보강)

위 내용은 2026-07-28 기준이다. 그 뒤 붙은 것만 여기 적는다.

### 라우트

```
/[lang]/concert            공연 일정 목록 — 달별로 묶고 id="2026-09" 앵커.
                           상세 179~213장을 직접 링크하는 크롤 경로이자
                           "9월 콘서트" 검색을 받는 페이지 (2026-09-01 신설)
```

`/concert` 목록이 없어서 콘서트 상세가 아티스트·공연장 페이지를 거쳐 깊이 3에서야
닿고 있었다. 홈은 카드 40개, 이 목록은 전부를 나열해 서로 겹치지 않는다.

### 검사 스크립트 — 전부 `npm run`으로 돈다

| 명령 | 무엇을 | 비고 |
| --- | --- | --- |
| `validate` | 데이터 무결성 + **last_updated가 git과 맞는가** | prebuild에 걸려 빌드를 막는다 |
| `validate:blog` | 글 길이·키워드·frontmatter | ko·ja 글자수, en 단어수 |
| `axe` | 접근성 | ⚠️ `reducedMotion: 'reduce'` 필수 |

`axe`에서 모션을 끄는 이유 — Reveal 애니메이션 때문에 그냥 재면 아직 나타나지 않은
요소(opacity: 0)의 색을 재서 거짓 양성이 쏟아진다(즉시 5건 / 3초 대기 3건 / 모션 끔 0건).

### 배포 제어

`vercel.json`의 `ignoreCommand` → `scripts/should-build.mjs`.

CHAT.md·prompts·wiki·docs만 바뀐 커밋은 빌드를 건너뛴다. 재배포는 공짜가 아니다 —
CDN 캐시가 통째로 무효화되고 크롤러가 다시 긁으면 전부 캐시 미스가 된다.
**`content/`와 `data/`는 화면에 그대로 나가므로 제외하지 않는다.**

이전 커밋을 못 찾으면(첫 배포·얕은 클론) **빌드하는 쪽으로** 실패한다. 반대로 하면
배포가 조용히 누락된다.

### ⚠️ 링크 prefetch

`next/link`의 기본 prefetch는 뷰포트에 들어오는 순간 RSC 페이로드를 당겨오고, 그 요청이
전부 ISR 읽기로 계산된다. **모든 페이지에 있는 공통 링크**(사이드바·푸터)에는
`prefetch={false}`를 준다. 실제로 사이드바 카드 4개 때문에 ISR Read가 요청의 4.1배까지
갔다(2026-09-07). 자세한 내용은 통합 위키 `nextjs.md`.

### ⚠️ 로컬 빌드가 Windows에서 실패한다

아티스트명 `"Weird Al" Yankovic`의 큰따옴표가 Windows 파일명에 못 들어가 export 단계에서
멈춘다. Vercel(리눅스)은 정상이고 해당 페이지도 라이브 200이다. 타입 검사와 페이지 생성
까지만 로컬에서 믿고, 최종 확인은 배포 후 라이브에서 한다.
