...
- [ ] **서치 콘솔 주소 변경 실행: gcalen.com → whenstage.com** (2026-07-29 등록)
      ...
      - 🔁 **되돌릴 수 있는 범위**: 301 리다이렉트는 Vercel 설정이라 언제든 해제 가능하고,
        도메인 등록 나이(2026-05-27~)도 리다이렉트 여부와 무관하게 계속 쌓인다. 다만 **서치 콘솔
        주소 변경 도구는 취소 절차가 구글 문서에 명시돼 있지 않다**(2026-07-29 확인). 나중에
        gcalen.com을 다시 쓸 가능성이 있다면 **301만 걸고 주소 변경은 보류**하는 편이 안전하다 —
        신호 이전의 대부분은 301이 하고, 되돌리기가 불확실한 건 주소 변경 쪽이다.
