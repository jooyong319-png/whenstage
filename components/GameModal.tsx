...
export function GameModal({ game, onClose, wishlist }: Props) {
  ...
  const ui = UI[lang];
  const t = CAL[lang];
  // 공연이 끝났으면 예매 마감일이 안 적혀 있어도 예매는 끝난 것이다(lib/types.ts hasEventEnded)
  const eventEnded = useEventEnded(game);
  const presaleEnded = useSaleWindowEnded(effectivePresaleEnd(game)) || eventEnded;
  const generalSaleEnded = useSaleWindowEnded(game.general_sale_end_datetime) || eventEnded;
  ...
  const dateStr = new Intl.DateTimeFormat(intlLocale, { year: 'numeric', month: 'long', day: 'numeric' }).format(parseDateOnly(game.release_date));
  const weekdayName = t!.weekdays[parseDateOnly(game.release_date).getDay()];
