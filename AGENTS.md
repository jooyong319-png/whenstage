...
```ts
{
  ...
  games: Array<{                    // 필드명은 games지만 실제로는 "공연/발매 항목" 배열
    ...
    updated_at?: string | null,                 // 'YYYY-MM-DD' 이 항목을 실제로 고친 날(§4-5, 선택)
  }>
}
```
...
⚠️ **`last_updated` / `last_researched_by`는 파일 최상위에만 쓴다 — 개별 항목에 넣지 말 것.**
2026-09-21 실측에서 이 두 필드가 **항목마다 복사돼** 있었다(ko 472개·en 590개·ja 444개 = 약 48KB).
코드는 파일 레벨 값만 읽으므로 항목 쪽은 아무 데서도 안 쓰이고, 클라이언트 컴포넌트로
그대로 직렬화돼 페이지 무게만 키운다. 네가 파일을 다시 쓸 때 **항목 안의 이 두 필드는 빼고**
저장하면 자연스럽게 정리된다(ko는 2026-09-21에 이미 정리됨).

### 중복 공연 병합 — "삭제 금지"의 유일한 예외 (2026-09-22)

같은 공연이 id만 다르게 두 번 올라가 있으면(날짜·공연장·아티스트·종류가 같음) **하나로 합친다.**
이건 정보를 버리는 게 아니라 흩어진 정보를 모으는 것이라 "데이터 보존 원칙"과 충돌하지 않는다.

- 남길 쪽: 콘텐츠(뉴스·블로그)에서 **더 많이 참조되는 쪽**, 동률이면 이미지가 있는 쪽
- 버리는 쪽의 값은 남길 쪽의 **빈 필드에 채우고**, 설명은 더 긴 쪽을 쓴다
- 버린 id를 가리키던 링크(`content/`·`data/`)는 남긴 id로 바꾼다
- **`data/concert-redirects.json`에 `{from, to}`를 추가한다** — `next.config.js`가 읽어 308로
  보낸다. 이미 색인·공유된 URL이 404가 되면 안 된다. 이 파일의 항목은 지우지 않는다

`npm run validate`가 내용 기준 중복을 잡는다(2026-09-22~).

### §4-5 `updated_at` — 이 항목을 실제로 고친 날 (선택)

`"YYYY-MM-DD"`. **사이트맵 `lastmod`가 이 값을 그대로 쓴다.**

- 이번 실행에서 **추가하거나 값을 고친 항목에만** 오늘 날짜를 넣는다.
- 🔴 **건드리지 않은 항목은 갱신하지 않는다.** 전체를 오늘로 덮으면 이 필드의 존재 이유가 사라진다.
- 기존 항목에 없는 것은 정상이다. 없으면 사이트맵이 `lastmod`를 **생략**한다.

**왜 생겼나 (2026-09-21)**: 예전엔 사이트맵이 로케일 데이터 갱신일을 그 로케일 상세
페이지 **전부**에 붙였다(ko 602장이 전부 같은 날짜). 안 바뀐 601장까지 "방금 수정됨"으로
신고하는 꼴이라, 구글이 lastmod 신호를 통째로 무시하게 되는 패턴이다. 틀린 날짜를
보내느니 안 보내는 편이 낫고, 실제로 고친 항목만 정확히 알리는 편이 제일 낫다.
...
- `data/artist-bios.json`은 **로케일별 중첩 구조**(`bios.ko` / `bios.en` / `bios.ja`)이고, 키는
  `normalizeArtistKey()` 결과와 일치해야 매칭된다

### 현재 상태 (2026-07-30 실측 — 백필의 표적을 정하는 숫자)
| 로케일 | 공연장 (색인 / 전체) | 아티스트 (색인 / 전체) |
|---|---|---|
| ko | **6 / 16** | 27 / 28 |
| en | **1 / 32** | 31 / 36 |
| ja | **11 / 50** | 36 / 36 |

### ⭐ 결론 — 백필의 색인 목표는 "공연장 페이지" 하나다
**아티스트 페이지는 이미 거의 다 해결돼 있다** — 100명 중 94명이 색인 대상이고, 그 대부분이
`bio` 덕분이다(`ws-artist-profile` 에이전트가 `artist-bios.json`을 채워 온 결과). 즉 아티스트 페이지를
색인시키려고 과거 공연을 채울 필요는 **없다.**

반면 **공연장은 98개 중 80개가 `noindex`** 다. 공연장 페이지에는 bio 같은 우회로가 없어서 오직
`events.length >= 2` 로만 판정되기 때문이다. **백필로 새로 색인시킬 수 있는 페이지는 이 80개가 전부다.**

### ⚠️ 함정 — "같은 투어의 다른 날짜"는 공연장 문제를 악화시킨다
백필 1건은 아티스트 페이지와 공연장 페이지를 동시에 건드린다. 그런데 같은 아티스트의 다른 도시 공연을
채우면 **공연장은 매번 새로운 1건짜리가 생겨서 `noindex` 페이지가 오히려 늘어난다.** EN의 공연장 32개 중
31개가 `noindex`인 게 정확히 이 현상이다 — 북미 투어는 도시마다 공연장이 다르다.

그래서 백필은 **"이미 등록된 공연장에서 열린 다른 공연"** 을 골라야 한다. 효율이 가장 높은 건
**대형 공연장에 과거 공연을 여러 건 몰아 넣는 것**이다. 공연이 많은 장소라 후보가 풍부하고, 한 사이클로
그 공연장 페이지를 확실히 2건 이상으로 만들 수 있다.

⚠️ **1건짜리 공연장을 새로 만드는 백필은 순 손실에 가깝다** — 상세 페이지 1개는 늘지만 `noindex`
페이지도 1개 늘어난다. 아티스트 페이지가 두꺼워지는 부수 효과는 있으나(콘텐츠 품질에는 도움),
**색인 페이지 수는 늘지 않는다**는 걸 알고 할 것.
