...
export interface Game {
  ...
  /**
   * 이 항목이 **마지막으로 실제로 바뀐 날**('YYYY-MM-DD'). 사이트맵 lastmod 전용.
   *
   * 왜 필요한가 (2026-09-21): 사이트맵이 콘서트 상세 1,587장에 **로케일당 한 날짜**를
   * 붙이고 있었다(ko 602장이 전부 같은 날). ko 데이터 파일이 갱신되면 안 바뀐 601장까지
   * "방금 수정됨"으로 신고하는 셈이라, 구글이 lastmod를 통째로 무시하게 되는 바로 그
   * 패턴이다(→ 통합 위키 seo.md "사이트맵 lastmod에 빌드 시각을 넣으면 신호가 죽는다").
   *
   * **선택 필드다.** 기존 839건은 값이 없고, 없으면 사이트맵이 lastmod를 **생략**한다 —
   * 틀린 날짜를 보내느니 안 보내는 게 낫다는 구글 권고 그대로다. 리서처가 앞으로
   * 손대는 항목부터 채우면 신호 품질이 점점 올라간다.
   *
   * ⚠️ 리서처는 **자기가 실제로 고친 항목만** 갱신한다. 매 실행마다 전체를 오늘로
   * 덮으면 지금 문제를 그대로 재현하는 것이다.
   */
