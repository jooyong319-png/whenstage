# [스케줄러용 프롬프트] 콘서트 캘린더 — JA(일본 국내) 리서처 Claude
...
1) 티켓 플랫폼(ぴあ/イープラス/ローソンチケット) 공연 상세 페이지의 og:image — web_fetch로 추출. 보통 공식 포스터라 신뢰도 높음.
...
4) 폴백(레이블/기획사 채널)을 쓴 항목은 CHAT.md에 **한국어로** "교체 대상"이라 기록.
