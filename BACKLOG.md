...
- 상태: 보류 (2026-07-24 — 클라우드 빌드 환경 제약으로 검증 미완, 아래 처리 기록 참고)
- 등록일: 2026-07-23
- 처리 기록(2026-07-24, 개발 담당): 스펙대로 루트 `og-image.png` 1개를 `git rm`으로 삭제 시도. **코드 변경 자체는 안전함이 확인됨** — `npx tsc --noEmit` 타입체크 통과, `grep -rn og-image`로 루트 파일을 읽는 코드 0건(모든 참조는 URL `/og-image.png` = `public/og-image.png`) 재확인. 그러나 **절대규칙(프로덕션 빌드 그린 확인 전 push 금지)의 `npm run build`를 이 클라우드 샌드박스에서 완주시키지 못함** — `next build`가 정적 페이지 생성(SSG, 270개) 단계에서 매번 멈춤(3회 시도: 한 번은 67/270에서, 한 번은 아티스트 페이지 렌더 직후에서 진행이 멈추고 이후 수 분간 파일 쓰기 0). 아티스트/상세 페이지는 로컬 JSON만 읽고 빌드타임 네트워크 호출이 없어 코드 원인으로 보기 어렵고, 샌드박스의 장시간 프로세스 제약(정적 생성 도중 정지/회수)으로 판단됨. **삭제 변경과는 무관한 환경 문제.** → 검증 못 한 코드를 push하지 않기 위해 로컬 변경을 되돌리고(원격 무영향) 상태만 `보류`로 기록. **다음 조치: 전체 빌드가 정상적으로 도는 환경(로컬 `d:/Gcalen/whenstage`의 `npm run build`, 또는 Vercel 프리뷰 빌드)에서 빌드 통과만 확인하면 그대로 삭제·push하면 됨.** 이 항목은 재시도 시 같은 클라우드 빌드 벽에 부딪히므로 자동 실행으로 반복 시도하지 말 것.
