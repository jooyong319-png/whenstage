...
- 상태: 완료 (2026-07-25, 커밋 d07dbc5)
- 처리 기록(2026-07-25, 개발 담당): 스펙대로 `openGraph`를 재정의하는 모든 로케일 페이지의 `generateMetadata`에서 페이지 고유 이미지가 없을 때 사이트 기본 OG 이미지(`/og-image.png`, `metadataBase` 절대경로 해석)로 폴백하도록 `images`를 항상 채움. 반복을 줄이려 `lib/i18nLabels.ts`에 `DEFAULT_OG_IMAGE = '/og-image.png'` 상수를 신설(각 페이지가 기존 `OG_LOCALE`와 같은 import에서 참조). 리스트/홈 5+1곳(`page.tsx`·`blog/page.tsx`·`artist/page.tsx`·`news/page.tsx`·`venue/page.tsx`·`guide/page.tsx`)은 `images: [DEFAULT_OG_IMAGE]` 명시, 고유 이미지 없을 수 있는 상세 4곳(`artist/[slug]`·`venue/[slug]`·`blog/[slug]`·`news/[slug]`)은 `undefined` 대신 `... ? [{ url: 고유 }] : [DEFAULT_OG_IMAGE]` 폴백으로 교체 — 총 11파일(10페이지+상수 1). `concert/[id]`는 이미 `images:[{url:ogImage}]` 폴백이 있어 불변(회귀 없음), `twitter`는 자식 미재정의로 레이아웃 상속 유지라 손 안 댐. **검증 = 타입체크 + 코드 리뷰**(§5-A 저위험 — 메타데이터 리터럴 추가로 로직·라우팅·타입·의존성 무영향, 무거운 `npm run build`는 §5-A대로 생략·Vercel 프리뷰 위임): 샌드박스 `/tmp`가 타 세션 잔재로 디스크 100% 차 `npm install`·`tsc`가 ENOSPC로 불가 → `/sessions`(여유 8G)에 작업트리 복사 후 클린 설치하여 `npm run typecheck` ✅ 오류 0 확인. 코드 리뷰: openGraph 재정의 페이지에 `images:.*undefined` 잔존 0건, `DEFAULT_OG_IMAGE` 사용 10곳, `concert/[id]` diff 0. 전체 빌드는 환경 되면 Vercel 프리뷰에서 재배포 후 `curl -s <url> | grep og:image`로 ko/en/ja 홈·인덱스·이미지없는 상세의 og:image(=`/og-image.png`) 존재 육안 확인 권장. 기술 함정 메모(통합 위키 소관): Next.js App Router는 자식 `generateMetadata`의 `openGraph`를 부모와 **얕은 병합(통째 대체)**하므로 하위 `images`를 딥머지하지 않는다 — 자식이 `openGraph`를 재정의하면 부모 `images`가 사라지니 이미지 없는 페이지는 명시 폴백 필수.
- 등록일: 2026-07-25
